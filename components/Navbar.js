    ...
    <nav className="bg-gray-900 text-white px-4 py-3 shadow-md sticky top-0 z-50">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-5">
          <Link href="/profile" className={`hover:text-primary-300 transition-colors font-medium ${router.pathname === '/profile' ? 'text-primary-400' : ''}`}>
            Profile
          </Link>
          <button 
      ...
      {isMenuOpen && (
        <div className="md:hidden mt-2 py-3 px-4 bg-gray-800 rounded-lg border border-gray-700 animate-fade-in">
          <div className="flex flex-col gap-3">
            <Link href="/profile" className={`hover:text-primary-300 transition-colors py-2 ${router.pathname === '/profile' ? 'text-primary-400' : ''}`}>
              Profile
            </Link>
            <button 
              onClick={handleLogout} 
